...
def train_model_improved(model, device, data_loader, criterion, optimizer, num_epochs, val_loader, batch_size,
    ...
    # 早停变量 - 使用验证准确率的移动平均
    best_val_acc = 0
    val_acc_history = []  # 存储历史验证准确率以计算移动平均
    ...
    for epoch in range(num_epochs):
        ...
        # 验证阶段结束后计算移动平均
        if len(val_acc_history) >= window_size:
            val_acc_history.pop(0)  # 移除最旧的值
        val_acc_history.append(val_accuracy)

        # 计算移动平均
        current_val_acc_avg = sum(val_acc_history) / len(val_acc_history)

        # 早停检查 - 使用验证准确率平均值
        if len(val_acc_history) >= window_size and current_val_acc_avg > best_val_acc:
            best_val_acc = current_val_acc_avg
            epochs_no_improve = 0
            # 保存最佳模型状态
        ...
        # 打印平均验证准确率
        if len(val_acc_history) >= window_size:
            print(f"Validation accuracy moving average ({window_size} epochs): {current_val_acc_avg:.5f}")
